...
const QUESTIONS: Record<Difficulty, Question[]> = {
  rookie: [
    ...
    { q: "A player catches a pass, takes two steps, and shoots a layup. Is this travelling?", opts: ["Yes — the gather step still counts, making it three steps total", "No — the two-step rule allows this after the gather", "Only in the paint where the gather rule doesn't apply", "Yes — but referees rarely call it at the youth level"], ans: "B" },
    ...
    { q: "What happens when a team accumulates too many fouls in a quarter?", opts: ["All further fouls result in automatic ejection of the fouling player", "The opposing team is awarded two technical free throws plus possession", "Bonus free throws on non-shooting fouls for the rest of the quarter", "The fouling team loses one of their remaining timeouts as a penalty"], ans: "C" },
    { q: "How many fouls does a player need to foul out?", opts: ["4", "5", "6", "3"], ans: "B" },
    { q: "What is a fast break?", opts: ["A quick set play called by the coach during a timeout", "Advancing the ball before the defense has time to set up", "A foul committed when a defender grabs a sprinting offensive player from behind", "A lineup change designed to bring in your best scorers quickly"], ans: "B" },
    { q: "What does boxing out mean?", opts: ["Trapping the ball handler in the corner using two defenders", "Positioning your body between an opponent and the basket to secure rebounds", "Setting a legal screen in the paint to free up a cutter", "Holding your ground in the post to receive an entry pass"], ans: "B" },
    { q: "What is a double dribble?", opts: ["Dribbling with two hands at once, or stopping your dribble and then starting it again", "Dribbling so hard the ball bounces above shoulder height", "Switching your dribbling hand mid-move without stopping the ball", "Losing control of the ball while dribbling and then recovering it yourself"], ans: "A" },
    { q: "What is an assist?", opts: ["A pass to a teammate that creates a scoring opportunity, even if the shot isn't made", "A pass that directly leads to a teammate making a basket", "Credit given when a defender helps stop a drive and forces a missed shot", "A stat given to the coach when a designed play results in a score"], ans: "B" },
    { q: "What is a turnover?", opts: ["When the defense rotates incorrectly and gives up an easy basket", "When the offense loses possession of the ball to the defense", "When a shot bounces off the rim and the offensive team recovers it", "When a player switches position with a teammate during a live play"], ans: "B" },
    { q: "Where must a player stand when shooting a free throw?", opts: ["Behind the three-point line, choosing any spot they are comfortable with", "On the free throw line, not crossing it until the ball hits the rim", "Anywhere inside the key as long as they don't move while shooting", "At the elbow of the free throw line, squared up to the basket"], ans: "B" },
    ...
    { q: "What does man-to-man defense mean?", opts: ["A defense where everyone sags toward the paint and covers the nearest offensive player", "Each defender is assigned to and responsible for one specific offensive player", "All five defenders collapse on the ball handler to force a turnover", "Defenders switch assignments after every screen to prevent mismatches"], ans: "B" },
    { q: "What is a rebound?", opts: ["Gaining possession of the ball after a missed shot", "An extra free throw awarded after an offensive player tips in a missed attempt", "A pass deliberately skipped off the floor to avoid a defender", "A sprint back to defend after losing possession on a turnover"], ans: "A" },
    { q: "What is the shot clock for?", opts: ["Tracking when substitutions are allowed and notifying the bench when to rotate", "Limiting how long the offense can hold the ball before attempting a shot", "Tracking how long each individual player has been on the court", "Signaling when each quarter is approaching its final two-minute mark"], ans: "B" },
    ...
    { q: "What does 'shooting pocket' mean?", opts: ["The spot on the court from which a player takes the majority of their attempts", "The position where a player naturally catches and shoots in one fluid motion", "A defensive technique for taking away a shooter's preferred hand side", "The gap between two defenders that an offensive player targets for a drive"], ans: "B" },
    { q: "What is goaltending?", opts: ["Blocking a shot attempt cleanly from behind the three-point line", "Illegally interfering with a shot on its downward arc or while it is on the rim", "Standing in the paint for more than three seconds while waiting for a pass", "Swatting a shot out of a player's hands before they fully release the ball"], ans: "B" },
  ],
  varsity: [
    { q: "A team inbounds from halfcourt, down 2, with 2 seconds left. Pass hits the backboard and goes in. Ruling?", opts: ["3 points — the play started behind the arc so it counts as three", "2 points and overtime — the ball was in play when time expired", "No basket — using the backboard on an inbound play is illegal", "2 points — backboard contact means the clock must restart from zero"], ans: "C" },
    ...
    { q: "What does help defense mean?", opts: ["Verbally directing teammates by calling out screens and opponent positions", "A defender leaving their assignment to stop an opponent who has beaten their teammate", "An intentional foul committed deliberately to prevent a fast-break layup", "Sagging several feet off your assignment to clog the paint and protect the rim"], ans: "B" },
    { q: "Difference between a blocking and charging foul?", opts: ["They are the same call — the difference is just which team the referee favors", "Blocking is called on the offense; charging is always called on the defense", "Blocking means the defender had no legal position; charging means the defender was set and the offensive player ran into them", "Charging only applies inside the restricted area under the basket"], ans: "C" },
    { q: "What is a skip pass?", opts: ["A quick bounce pass thrown before the defense can rotate to close out", "A long pass across the court to the weak side, bypassing intermediate defenders", "A no-look pass thrown in the opposite direction from where the passer is looking", "A behind-the-back pass used to hit a cutter breaking along the baseline"], ans: "B" },
    { q: "In a 2-3 zone, what do the numbers represent?", opts: ["Two defenders at the top of the key, three spread across the baseline", "Two forwards protecting the paint, three guards covering the perimeter", "Two defenders on the ball side, three rotating to the weak side on each pass", "Two defenders at the elbows, three stationed along the three-point line"], ans: "A" },
    { q: "What is motion offense?", opts: ["A series of scripted plays run in sequence based on what the defense shows", "A free-flowing offensive system where players read the defense and make decisions without predetermined plays", "An offense that only pushes pace on fast breaks and sets up in transition", "An offense built entirely around isolating your best scorer one-on-one each possession"], ans: "B" },
    { q: "What is a back screen?", opts: ["A screen set behind the ball handler to allow a guard to curl into the paint", "A screen set on a defender positioned between the screener and basket, freeing a teammate to cut toward the ball", "A screen set along the baseline to free a shooter coming off the corner", "A screen set near halfcourt to create separation for a full-court skip pass"], ans: "B" },
    { q: "What is an 'and-one'?", opts: ["A three-point play where a player draws a foul on a made three-point shot", "Making a basket while being fouled, earning one additional free throw", "An isolation play specifically designed to draw contact and stop the clock", "An extra free throw added when a player converts both shots in the bonus"], ans: "B" },
    ...
    { q: "What is a give-and-go?", opts: ["Passing to a teammate and immediately cutting to the basket to receive a return pass", "Passing and then setting a screen for the teammate you just passed to", "A defensive maneuver where the help defender takes the ball handler while the primary defender rotates away", "A play drawn in a timeout where the inbounder runs the ball after faking the pass"], ans: "A" },
    { q: "What does 'weak side' mean?", opts: ["The side of the court where your least effective offensive players are stationed", "The side of the court opposite to where the ball currently is", "The side of the court closest to the team's bench during a home game", "The side of the court where the defense is numerically outnumbered"], ans: "B" },
    { q: "What is a closeout?", opts: ["The final two minutes of a game when a leading team protects its advantage", "A defender sprinting to contest an open shooter while staying balanced enough not to foul", "A full-court press triggered after a made basket to apply immediate defensive pressure", "Two defenders collapsing simultaneously on the ball handler to force a turnover"], ans: "B" },
    ...
    { q: "What does 'running a play off timeout' mean?", opts: ["The coach designs a specific play for the team to execute immediately when play resumes", "Players use the timeout to recover stamina before returning to free-flowing offense", "The team switches to a zone defense for the first few possessions after the break", "The best player on the floor signals their preferred play and teammates adjust around them"], ans: "A" },
    { q: "What is a defensive rotation?", opts: ["Substituting multiple defenders simultaneously to prevent fatigue in crunch time", "Defenders adjusting their positions in response to offensive movement to maintain proper coverage", "A team-wide switch from zone to man-to-man as the offense crosses halfcourt", "A scripted sequence where each defender rotates one spot clockwise each possession"], ans: "B" },
    { q: "What does 'penetrate and kick' mean?", opts: ["Kicking the ball ahead to a sprinting teammate — a legal pass technique on the move", "A ball handler driving into the paint to collapse the defense, then passing out to an open perimeter shooter", "A fast break where the lead guard drives until contact is drawn, then kicks to a trailing big", "A post entry where the big catches the ball and immediately kicks back to reset the offense"], ans: "B" },
    ...
    { q: "What does 'playing off the ball' mean?", opts: ["Deliberately avoiding receiving the ball to give your teammates more room to operate", "The movement, cutting, screening, and positioning of players who do not currently have the ball", "A defensive concept where players without a ball assignment sag toward the paint to protect the rim", "Stationing your best shooters in the corners to stretch the defense and create driving lanes"], ans: "B" },
    { q: "What is a secondary break?", opts: ["The halftime intermission during which the coaching staff makes their primary adjustments", "After the initial fast break transition, the offense continues attacking quickly before the defense can fully set", "A second timeout called right after the first to extend preparation before a final play", "A substitution pattern designed to keep fresh legs on the floor during the final two minutes"], ans: "B" },
  ],
  elite: [
    { q: "In a 2-3 zone, there's a skip pass to the weak-side corner. Which defender contests?", opts: ["The weak-side guard, who drops from the perimeter to take the corner shooter", "The center, who slides across the lane to protect against the corner three", "The weak-side wing, who is responsible for that corner in 2-3 zone coverage", "The strong-side forward, who rotates across the lane once the ball moves"], ans: "C" },
    ...
    { q: "What is a horns set?", opts: ["Two bigs stationed at the elbows with guards in the corners, creating multiple simultaneous attack options", "A fast break alignment where two wings sprint ahead while the point guard follows with the ball", "A full-court press that funnels the ball handler to one side and doubles from behind", "An isolation set that clears one side of the floor for your best one-on-one scorer"], ans: "A" },
    { q: "Purpose of a DHO (dribble hand-off)?", opts: ["Slowing the pace and resetting the offense when the initial action hasn't created an opportunity", "Creating a mismatched guard advantage by forcing the defense into a difficult switch-or-hedge decision", "Delivering the ball to a post player through a hand-off rather than a lob or bounce pass", "Running down the shot clock before initiating the primary play in the final seconds"], ans: "B" },
    { q: "What is a scramble switch?", opts: ["A practice drill that simulates chaotic transition situations to improve defensive communication", "An emergency defensive adjustment where players switch assignments after a screen has already broken normal coverage", "An offensive set triggered by a hand signal when the primary action has been taken away by the defense", "A press break designed to handle a surprise full-court trap by scrambling to designated open spots"], ans: "B" },
    ...
    { q: "'Playing the gaps' in zone defense means?", opts: ["Positioning between offensive players to deny passing lanes and generate deflections", "Positioning just behind your zone responsibilities to take away skip passes to the corners", "Doubling the post player whenever the offense overloads a gap in the zone", "Trapping the ball handler at the top of the key each time they enter the front of the zone"], ans: "A" },
    { q: "'Paint touches create offense' means?", opts: ["A fouling strategy targeting post players who are weak free throw shooters", "Getting the ball into the paint forces the defense to collapse, opening up perimeter shooters", "The post player should always look to shoot first and only kick out as a last resort", "Driving into the paint is primarily about drawing fouls rather than scoring at the rim"], ans: "B" },
    { q: "What is a flare screen?", opts: ["A screen set at halfcourt to free a ball handler coming off a pick near the logo", "A screen set away from the ball that frees a shooter moving outward toward the three-point line or corner", "A baseline screen where the cutter curls underneath the basket toward the near corner", "A post-up screen inside the paint designed to pin a smaller defender on a bigger offensive player"], ans: "B" },
    ...
    { q: "What is a 'dunker spot'?", opts: ["The spot where the team's best finisher positions to receive alley-oop lob passes", "A position inside the paint near the baseline opposite the ball, stretching the defense and creating lob or dump-off options", "The low post position where a center backs down a defender for a power move to the rim", "The short corner area just inside the three-point line used for mid-range pull-up jumpers"], ans: "B" },
    { q: "What is 'chin action'?", opts: ["A defensive signal where the point guard taps their chin to call a half-court trapping scheme", "The point guard dribbling toward the big at the elbow, triggering a series of cuts and screens for the rest of the offense", "A fast break signal where the point guard taps their chin after a defensive rebound to push the pace", "A press break alignment where the ball handler calls the play by touching their chin to the defender"], ans: "B" },
    { q: "What is the 'nail position' in defense?", opts: ["The defender assigned to the opponent's best player and responsible for following them anywhere on the floor", "The help defender positioned at the middle of the free throw line, ready to stop drives from either side", "The most exposed positional assignment, typically given to the player who cannot guard on the perimeter", "A guard who drops into the post to protect against high-low passes when a big catches at the elbow"], ans: "B" },
    ...
    { q: "'Going under a screen' on defense means?", opts: ["Physically ducking under the screener's arms to maintain contact with the ball handler", "The defender passes behind the screener on the basket side, accepting space to prevent the ball handler from turning the corner", "Switching assignments with a teammate as the ball handler uses the screen and rolling your man", "Aggressively jumping to the level of the screen to take away the ball handler's downhill drive"], ans: "B" },
    { q: "What is a short roll?", opts: ["A screen set by a smaller, more mobile player specifically designed to confuse a bigger, slower defender", "The screener stopping short in the paint after the pick, positioning as a secondary ball handler rather than rolling to the rim", "A quick post entry pass to a big who immediately turns and shoots before the defense can react", "A fast break variation where the trailing big catches a skip pass and attacks off the dribble"], ans: "B" },
    { q: "What does 'early offense' mean?", opts: ["Beginning offensive sets before the shot clock starts to get an early read on how the defense is aligned", "Pushing pace immediately after a made basket or defensive rebound, attacking before the opponent can get set", "Running the specific first play of the game drawn up during pre-game preparation", "Calling a quick timeout to reset the offense before the defense can establish their coverage"], ans: "B" },
    ...
    { q: "What does 'spacing' mean and why does it matter?", opts: ["The physical distance between players on the bench during a timeout to allow the coach to walk and communicate freely", "Deliberately positioning offensive players to stretch the defense, creating driving lanes, post gaps, and passing angles", "How far each player stands from the three-point line to maximize their ability to catch and shoot", "Keeping all five players spread evenly across the halfcourt so no two teammates are within ten feet of each other"], ans: "B" },
    { q: "What is a ghost screen?", opts: ["A screen set so quickly and subtly that illegal contact goes undetected by the referee", "A player fakes setting a screen and peels away — the ball handler attacks off the fake while the screener opens up as an immediate scoring option", "A screen set out of bounds to legally free a receiver on a sideline inbound play", "A screen set near halfcourt designed to free a cutter for a full-court lob pass to the rim"], ans: "B" },
  ],
};
