...
export function ProfilePage() {
  ...
  // All-season stats for a single player (used for season-specific display stats)
  ...
  const careerTotalsForCard =
    careerGames > 0
      ? {
          ...
          totalFieldGoalsMade:      src?.totalFieldGoalsMade      ?? 0,
          totalFieldGoalsAttempted: src?.totalFieldGoalsAttempted ?? 0,
          totalThreesMade:          src?.totalThreesMade          ?? 0,
          totalThreesAttempted:     src?.totalThreesAttempted     ?? 0,
          totalFreeThrowsMade:      src?.totalFreeThrowsMade      ?? 0,
          totalFreeThrowsAttempted: src?.totalFreeThrowsAttempted ?? 0,
          fieldGoalPct:  src?.fieldGoalPct  ?? 0,
          threePointPct: src?.threePointPct ?? 0,
          freeThrowPct:  src?.freeThrowPct  ?? 0,
          ...
          {(() => {
            ...
            return (
              <Link href="/arcade/shot-clock-scramble">
                ...
                >
                  <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(245,158,11,0.15) 0%, transparent 60%)" }} />
                  <div className="flex items-center justify-between px-4 pt-4">
                    <div className="flex items-center gap-2">
                      <span style={{ fontSize: 18 }}>⏰</span>
                      <span className="font-display text-sm tracking-widest text-white/60 uppercase">Shot Clock Scramble</span>
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 border border-amber-500/60" style={{ color: "#f59e0b", background: "rgba(245,158,11,0.1)" }}>
                      Trivia
                    </span>
                  </div>
                  {scs ? (
                    <>
                      <div className="flex items-center gap-4 px-4 py-3">
                        <div>
                          <p className="text-[9px] font-bold uppercase tracking-widest text-white/35 mb-0.5">High Score</p>
                          <p className="font-display leading-none" style={{ fontSize: "clamp(52px,12vw,72px)", color: "#f59e0b" }}>{scs.bestScore}</p>
                        </div>
                        <div className="flex-1 flex justify-end items-center pr-2">
                          <div className="relative w-16 h-16">
                            <div className="absolute inset-0 rounded-full border-2 border-amber-800/50" />
                            <div className="absolute inset-1 rounded-full border-2 border-amber-600/40" style={{ animation: "ping 1.8s cubic-bezier(0,0,0.2,1) infinite" }} />
                            <div className="absolute inset-0 flex items-center justify-center">
                      ...
                      <div className="flex border-t border-white/8" style={{ background: "rgba(0,0,0,0.35)" }}>
                        ...
                        <div className="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-amber-400 group-hover:text-amber-300 transition-colors border-l border-white/8">Play →</div>
                      </div>
                    </>
                  ) : (
                    <div className="px-4 pb-4 flex items-center justify-between">
                      <p className="text-sm text-white/30 italic">No games yet</p>
                      <span className="text-[11px] font-black uppercase tracking-widest text-amber-400 group-hover:text-amber-300 transition-colors">Play Now →</span>
                    </div>
                  )}
                </div>
              </Link>
